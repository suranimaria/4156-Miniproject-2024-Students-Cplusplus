 ...
 * Department and an HTTP 200 response or, an appropriate message indicating the
 * proper response.
 */
void RouteController::retrieveDepartment(const crow::request& req,
                                         crow::response& res) {
  try {
 ...
 * wishes to find the course in.
 *
 * @param courseCode A {@code int} representing the course the user wishes
 *                   to retrieve.
 *
 * @return           A crow::response object containing either the details of
 * the course and an HTTP 200 response or, an appropriate message indicating the
 *                   proper response.
 */
void RouteController::retrieveCourse(const crow::request& req,
  ...
  try {
    if (!req.url_params.get("deptCode") || !req.url_params.get("courseCode")) {
      res.code = 400;
      res.write("Both department code and course code must be included in the request.");
      res.end();
      return;
    }
    ...
    if (deptIt == departmentMapping.end()) {
      ...
      if (courseIt == coursesMapping.end()) {
        ...
        res.code = 200;
        res.write(
            courseIt->second->display());  // Use dot operator to access method
      }
    }
    res.end();
  } catch (const std::exception& e) {
 ...
 *                   to retrieve.
 *
 * @return           A crow::response object containing either the requested
 * information and an HTTP 200 response or, an appropriate message indicating
 * the proper response.
 */
 ...
 *
 * @param deptCode     A {@code string} representing the department the user
 * wishes to find number of majors for.
 *
 * @return             A crow::response object containing either number of
 * majors for the specified department and an HTTP 200 response or, an
...
void RouteController::getMajorCountFromDept(const crow::request& req,
  ...
  try {
    ...
    if (deptIt == departmentMapping.end()) {
      ...
      res.code = 200;
      res.write(
          "There are: " + std::to_string(deptIt->second.getNumberOfMajors()) +
 ...
 *
 * @return          A crow::response object containing either department chair
 * of the specified department and an HTTP 200 response or, an appropriate
...
void RouteController::identifyDeptChair(const crow::request& req,
  ...
  try {
    ...
    if (deptIt == departmentMapping.end()) {
      ...
      res.code = 200;
      res.write(
          deptIt->second.getDepartmentChair() +
 ...
 *
 * @return           A crow::response object containing either the location of
 * the course and an HTTP 200 response or, an appropriate message indicating the
 *                   proper response.
 */
    ...
    if (deptIt == departmentMapping.end()) {
      ...
      if (courseIt == coursesMapping.end()) {
        ...
        res.write(course->getCourseLocation() +
                  " is where the course is located.");  // Use dot operator to
                                                        // call method
      }
    }
    res.end();
 ...
 *                   to find information about.
 *
 * @return           A crow::response object containing either the course
 * instructor and an HTTP 200 response or, an appropriate message indicating the
 * proper response.
 */
    ...
    if (deptIt == departmentMapping.end()) {
      ...
      if (courseIt == coursesMapping.end()) {
        ...
        res.write(course->getInstructorName() +
                  " is the instructor for the course.");  // Use dot operator to
                                                          // call method
      }
    }
    res.end();
 ...
 *
 * @return           A crow::response object containing either the details of
 * the course timeslot and an HTTP 200 response or, an appropriate message
...
void RouteController::setEnrollmentCount(const crow::request& req,
                                         crow::response& res) {
  try {
    if (!req.url_params.get("deptCode") || !req.url_params.get("courseCode") || !req.url_params.get("count")) {
      res.code = 400;
      res.write("Department code, course code and new count must ALL be included in the request.");
      res.end();
      return;
    }
...
void RouteController::setCourseLocation(const crow::request& req,
                                        crow::response& res) {
  try {
    if (!req.url_params.get("deptCode") || !req.url_params.get("courseCode") || !req.url_params.get("location")) {
      res.code = 400;
      res.write("Department code, course code and new location must ALL be included in the request.");
      res.end();
      return;
    }
...
void RouteController::setCourseInstructor(const crow::request& req,
                                          crow::response& res) {
  try {
    if (!req.url_params.get("deptCode") || !req.url_params.get("courseCode") || !req.url_params.get("instructor")) {
      res.code = 400;
      res.write("Department code, course code and new instructor must ALL be included in the request.");
      res.end();
      return;
    }
...
void RouteController::setCourseTime(const crow::request& req,
                                    crow::response& res) {
  try {
    if (!req.url_params.get("deptCode") || !req.url_params.get("courseCode") || !req.url_params.get("time")) {
      res.code = 400;
      res.write("Department code, course code and new time must ALL be included in the request.");
      res.end();
      return;
    }
